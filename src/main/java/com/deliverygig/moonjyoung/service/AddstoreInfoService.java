...
public class AddstoreInfoService {
    ...
    // 가게 기본정보 등록 
    public Map<String, Object> addStore(AddstoreInfoVo data, HttpSession session) {
        ...
        StoreInfoEntity entity = new StoreInfoEntity(null, data.getSiName(), data.getSiDiscount(), null,
                data2.getOiSeq(), null);
        sRepo.save(entity);
    ...
    // 가게 기본정보 수정 
    public Map<String, Object> UpdateStore(UpdateStoreVO data, Long siSeq, String type, HttpSession session) {
        Map<String, Object> resultMap = new LinkedHashMap<String, Object>();
        OwnerEntity data2 = (OwnerEntity) session.getAttribute("loginOwner");
        if (session.getAttribute("loginOwner") == null) {
            resultMap.put("status", false);
        ...
        // 가게 할인율 수정
         if (type.equals("storeDiscount")) {
            ...
            if (0 > data.getUpdatesiDiscount() || data.getUpdatesiDiscount() > 1) {
                resultMap.put("status", false);
